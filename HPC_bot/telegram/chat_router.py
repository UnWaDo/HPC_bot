...
ACCESS_GRANTED = ('Доступ предоставлен. Для завершения регистрации '
...
async def joined_chat(event: ChatMemberUpdated):
    ...
    if event.from_user.username != config.bot.admin_name[1:]:
        await event.bot.send_message(event.chat.id, ILLEGAL_ADD)
        await event.bot.leave_chat(event.chat.id)
        left = True

    if left:
        await log_message(event.bot,
...
async def left_chat(event: ChatMemberUpdated):
    ...
    if event.chat.type == 'private':
        ...
        await log_message(event.bot,
                          text=BLOCKED.format(bot_name=config.bot.bot_name,
                                              user=create_user_link(user)))
        return
    await log_message(event.bot,
                      text=DELETED.format(bot_name=config.bot.bot_name,
                                          chat_name=event.chat.full_name))
