...
class Runner(BaseModel):
    ...
    def _validate_args(args: List[str], allowed_args: List[str] = []):
        for arg in args:
            if not any(re.fullmatch(r, arg) for r in allowed_args):
                raise ValueError(f'Argument {arg} is not in the list of allowed arguments')
    ...
    @classmethod
    def get_default_command(cls, field: List[str], info: FieldValidationInfo) -> str:
        if 'allowed_args' not in info.data:
            raise ValueError('Require allowed_args to validate default_command')
    ...
    def create_command(self, args: List[str] = None, filename: str = '') -> str:
        if args is not None:
    ...
    def split_command(self, command: str) -> List[str]:
        ...
        if args[0] != self.program:
            raise ValueError(f'Program name must be {self.program}, got {args[0]} instead')
